...
namespace Memory
{
    internal static class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            //Arrays
            int[] a = { 0, 0 }; // 1 coordinate (x,y)
            int[] b = { 0, 0 }; // 2.coordinate (x,y)
            char[] symbols =
            {
                '☺', '☻', '♦', '♠', '♥', '♣', '♫', '☼', '©', '&', '▲', '►', '▼', '◄', '#', '§', '$', '£', '✿', '✌', '☁',
                '❈', '☂', '☎', '▣', '☃'
            };
            var blancField = new char[size[0], size[1]];
            var cardField = new char[size[0], size[1]];
            ...
            while (!CheckIfFinish(blancField, size)) blancField = playRound(blancField, cardField, size, sound);
        ...
        private static void ThisIsTheEnd(bool sound)
        {
            Jukebox(sound);
        ...
        {
            //Songs
            var Skyfall = new SoundPlayer(path + "sounds\\Skyfall.wav");
            var InTheEnd = new SoundPlayer(path + "sounds\\InTheEnd.wav");
            var GoodThingsComeToAnEnd = new SoundPlayer(path + "sounds\\GoodThingsComeToAnEnd.wav");
            ...
            switch (Song.Next(1, 4))
            {
                case 1:
                    ...
                    break;
                case 2:
                    if (sound) InTheEnd.Play();

                    Console.WriteLine(
                        "I tried so hard and got so far\nBut in the end, it doesn't even matter\nI had to fall to lose it all\nBut in the end, it doesn't even matter");
                    break;
                case 3:
                    if (sound) GoodThingsComeToAnEnd.Play();
        ...
        /////////////////////////////
        private static void ShowField(char[,] field, int[] size)
        {
        ...
        private static char[,] MakeCleanArray(int[] size)
        {
        ...
        private static int[] InputCardLocation(char[,] blancField, int[] size)
        {
            var iArray = new int[2];
            bool number;
            var error = false;
            ...
            do
            {
                ...
                try
                {
                    var cArray = Console.ReadLine().ToCharArray();
                    ...
                    {
                        number = false;
                        Console.WriteLine("This is not a number or it is outside the field");
                ...
                {
                    Console.Write("you have to input coordinates: ");
                    number = false;
                }
            } while (!number);
        ...
        public static char[,] playRound(char[,] blancField, char[,] cardField, int[] size, bool sound)
        {
            ...
            if (!(cardField[cardA[0], cardA[1]] == cardField[cardB[0], cardB[1]]))
            {
                ...
                var Duck = new SoundPlayer(path + "sounds\\WhatTheDuck.wav");
                if (sound) Duck.Play();
            ...
            {
                var Yeah = new SoundPlayer(path + "sounds\\Yeah.wav");
                if (sound) Yeah.Play();
